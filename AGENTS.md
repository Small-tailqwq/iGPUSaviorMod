...
## MCP 工具
### ilspy-mcp（程序集反编译）
- 工具：`decompile_type` / `search_members_by_name` / `get_type_members`
- 注意 `NoteUI` / `SelectPageListUI` 等不属于 `Bulbul.` 命名空间，使用非限定名搜索

### unity-explorer（运行时 UI 检视）
- BepInEx 插件版，游戏内按 F7 打开
- 工具：`search_elements` / `inspect_element` / `get_ui_hierarchy`
